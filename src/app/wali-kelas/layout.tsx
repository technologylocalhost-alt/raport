...
export default function WaliKelasLayout({ children }: WaliKelasLayoutProps) {
  ...
  const [userRole, setUserRole] = useState<string>('WALI_KELAS');
  ...
  useEffect(() => {
    ...
    async function bootstrapSession() {
      try {
        ...
        setUserRole(sessionUser.role || 'WALI_KELAS');
  ...
  useEffect(() => {
    ...
    async function fetchAllowedMenus() {
      try {
        const { allowedPaths, hasRestrictions } = await fetchAllowedMenuPaths('wali-kelas');
                ...
                <>
                  {isExtraAccessSection ? (
                    <button
                      ...
                      className="w-full px-2 sm:px-3 py-1 flex items-center justify-between text-xs font-semibold text-emerald-400 uppercase tracking-wider hover:text-emerald-200 transition-colors"
                    >
